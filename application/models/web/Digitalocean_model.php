...
class Digitalocean_model extends CI_Model {
	function upload_photo($filename,$path, $storedName = ''){
		$temp = explode(".", $_FILES["$filename"]["name"]);
		$newfilename = $storedName != "" ? $storedName : ($temp[0]."_".round(microtime(true)) . '.' . end($temp));
		...
		if($filepath !=''){
			...
			$sanitizedFileName = preg_replace('/[\s ]+/', '_', $newfilename);
            $sanitizedFileName = preg_replace('/[^A-Za-z0-9\-\_\.]/', '', $sanitizedFileName); // Remove any other special characters
            
			$client->putObject([
				 'Bucket' => 'tgu-filer',
				 'Key'    => $path.$sanitizedFileName,
				 'Body'   => file_get_contents($filepath),
				 'ACL'    => 'private',
				 'Metadata'   => array(
				 'x-amz-meta-my-key' => $sanitizedFileName
				 )
			]);
	...
	function upload_photo_multiple($filename,$path){
		$fileName = "";
		if(!empty($_FILES["$filename"]['name'][0]) && $_FILES["$filename"]['name'][0] != ""){
			$files = $_FILES;
	...
	/*---------------------------------------------------------------*/
		...
		*/
		function upload_photo_direct($filename,$filepath,$path){
