...
class Center_details_model extends CI_Model{
	...
	public function set_center($photo, $adharcard, $mou,$undertaking)
	{
		if ($photo == "") {
	...
	public function set_registration($photo, $noc, $identity_file, $permission_letter, $undertaking,$affidavit_file, $old_migration){ 
		...
		if ($this->input->post('admission_type') == "2") {
...
// 			echo "<pre>";print_r($fees_result);exit;
	...
	public function get_all_added_subject($length, $start, $search)
	{
		...
		$this->db->where('tbl_stream.status', '1');
	...
	public function get_exam_status($year_sem){
		$student = $this->get_result_student(); 
		if (!empty($student)) {
			$this->db->where('is_deleted', '0');
			$this->db->where('status', '1');
	...
	public function get_student_fees($session, $course, $stream, $country, $course_mode, $year_sem){
		...
		if ($center->payment_term == "1") {
			if($course_mode == '1'){
				...
				if($year_sem == '2' || $year_sem == '4' || $year_sem == '6' || $year_sem == '8'){
					$fees = $fees;
				}else{
					$fees = 0;
				}
	...
	public function check_exam_open($course_mode, $session){
	    //echo $session;exit;
    	$center_arr = array('23','31','1','12','154');
    	...
    	if(in_array($this->session->userdata('center_id'),$center_arr)){
    		...
    		}elseif($this->session->userdata('center_id') == '154'){
    		    $this->db->where('setup_id', '5'); 
    		}else if($this->session->userdata('center_id') == '31'){ 
    		    $setup_id = array('2','6');
    		    $this->db->where_in('setup_id',$setup_id); 
    		    //$this->db->where_in('setup_id','2'); 
    		}else if($this->session->userdata('center_id') == '1' || $this->session->userdata('center_id') == '12'){ 
    		    $this->db->where('setup_id', '6'); 
    		}
	...
	public function set_examination_form()
	{
		...
		if(in_array($this->session->userdata('center_id'),$center_arr)){
			if($this->session->userdata('center_id') == "23"){
			    ...
			    if($student_data->session_id == '3'){
			        $this->db->where_in('setup_id','6');
			    }else{
			        $this->db->where_in('setup_id','2');
			    }
			}else if($this->session->userdata('center_id') == "1" || $this->session->userdata('center_id') == "12"){
...
// 		return $result->row();
		...
		$this->db->select('tbl_student.*, states.name as state_name, cities.name as city_name');
		$this->db->where('tbl_student.id', base64_decode($this->uri->segment(3)));
	...
	public function get_single_student_details_new($id){
		...
		$this->db->select('tbl_student.*,cities.name as city_name,states.name as state_name');
	...
	public function get_exam_form_fees($exam_id){
	    $this->db->select('tbl_examination_form.student_id,tbl_student_fees.id');
	    $this->db->where('tbl_examination_form.id',$exam_id);
	...
	public function update_student_identity_soft($photo, $identity_file, $signature,$undertaking)
	{
		$data = array(
			'identity_softcopy' 	=> $identity_file,
			'photo' 				=> $photo,
	...
	{
		$this->db->select('tbl_center_industry_assessment_form.*,tbl_student.student_name,tbl_student.course_mode,tbl_course.course_name,tbl_stream.stream_name');
	...
	public function get_balance_student(){ 		
		$this->db->select('tbl_student.*,countries.name as country_name,states.name as state_name,cities.name as city_name,tbl_balance_student.balance_fees,tbl_balance_student.id as balance_id'); 		
		$this->db->where('tbl_balance_student.is_deleted','0'); 		
		$this->db->join('tbl_student', 'tbl_student.enrollment_number = tbl_balance_student.enrollment_no');   		
		$this->db->join('countries', 'countries.id = tbl_student.country'); 		
		$this->db->join('states', 'states.id = tbl_student.state'); 		
		$this->db->join('cities', 'cities.id = tbl_student.city'); 		
		$result = $this->db->get('tbl_balance_student'); 		
		return $result->result(); 	
	} 	
	public function get_balance_payment_single(){ 		
		$this->db->select('tbl_student.*,countries.name as country_name,states.name as state_name,cities.name as city_name,tbl_balance_student.balance_fees,tbl_balance_student.id as balance_id'); 		
		$this->db->where('tbl_balance_student.id',$this->uri->segment(2)); 		
		$this->db->where('tbl_balance_student.is_deleted','0'); 		
		$this->db->join('tbl_student', 'tbl_student.enrollment_number = tbl_balance_student.enrollment_no');   		
		$this->db->join('countries', 'countries.id = tbl_student.country'); 		
		$this->db->join('states', 'states.id = tbl_student.state'); 		
	...
	public function get_single_subject_details(){
	    $this->db->where('is_deleted', '0');
		$this->db->where('status', '1');
		$this->db->where('id',$this->uri->segment(2));
		$this->db->where('center_id',$this->session->userdata('center_id'));
	...
	public function get_selected_course_streams($course){
	...
	public function get_selected_course_stream_mode($course, $stream){
    	$this->db->where('is_deleted', '0');
		$this->db->where('course', $course);
		$this->db->where('stream', $stream);
		$result = $this->db->get('tbl_course_stream_relation');
	...
	public function get_selected_course_stream_subject($course, $stream, $mode, $year_sem){
    	$this->db->where('is_deleted', '0');
		$this->db->where('course', $course);
		$this->db->where('stream', $stream);
	...
	{ 
		$this->db->select('tbl_course_stream_relation.course,tbl_course_stream_relation.stream,tbl_stream.id,tbl_stream.stream_name,tbl_center_fees.center_id');
		$this->db->where('tbl_course_stream_relation.course', $this->input->post('course'));
		$this->db->where('tbl_center_fees.course_id', $this->input->post('course'));
		$this->db->where('tbl_center_fees.center_id', $this->session->userdata('center_id'));
		$this->db->where('tbl_center_fees.is_deleted', '0');
		//$this->db->join('tbl_center_fees', 'tbl_center_fees.course_id = tbl_course_stream_relation.course');
		$this->db->join('tbl_center_fees', 'tbl_center_fees.stream_id = tbl_course_stream_relation.stream');
		$this->db->join('tbl_stream', 'tbl_stream.id = tbl_center_fees.stream_id');
		//$this->db->group_by('tbl_center_fees.course_id');
		$result = $this->db->get('tbl_course_stream_relation');
		echo json_encode($result->result());
