				...
				<tbody>
						...
						if(!empty($payment)){ foreach($payment as $payment_result){?>							
							<tr>
								...
								<td>
									<a type="button" title="Edit" data-toggle="tooltip" href="<?=base_url()?>guide_payments_blended/<?=$this->uri->segment(2)?>/<?=$payment_result->id?>"  class='btn btn-success btn-sm'><i class='mdi mdi-table-edit'></i></a>
									<a onclick="return confirm('Are you sure, you want to delete this record permanently?')" data-toggle="tooltip" title="Permanently Delete" href="<?=base_url()?>delete/<?=$payment_result->id?>/tbl_guide_payment_blended" class='btn btn-danger btn-sm'><i class='mdi mdi-delete'></i></a>
