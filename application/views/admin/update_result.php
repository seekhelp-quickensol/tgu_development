					...
					<?php if(!empty($result)){?>
						<form name="edit_result_form" id="edit_result_form" style="margin-top: 0px;" onsubmit="return validation()" method="post">  
							<table class="detailTable" align="center"   cellspacing="5" cellpadding="4">
								...
								<tr><td>Year/Sem</td>
									<td class="info1">
									<select name="year_sem" id="year_sem" class="detail_combo">
									<option value="">Year/Sem</option>
										<?php for($ys=1;$ys<=12;$ys++){?>
										
										<option value="<?=$ys?>" <?php if($result->year_sem == $ys){?>selected="selected"<?php }?>><?=$ys?></option> 
										<?php }?>
									</select>
									</td>
									<td class="reqfield">&nbsp;</td>
							...
							<table  align="center" class="table-striped table-bordered" cellpadding="5">
								...
								<?php 
									$i=1;
									$subject = $this->Exam_model->get_selected_subject_for_result($result->id);
									if(!empty($subject)){
