              ...
              <div class="card">
                <div class="card-body">
                  ...
                  <form class="forms-sample" name="bank_form" id="bank_form" method="post" enctype="multipart/form-data">
                     ...
                     <?php 
                        $fees_details = $this->Student_model->get_re_registration_fees_details($stu_data['id'],$stu_data['current_year_sem']);
				 ...
				 <input type="hidden" name="fees_hidden_id" value="<?php if(!empty($fees_details)){ echo $fees_details->id;}?>">
				  ...
				  <div class="col-md-3">
                    <div class="form-group">
                      ...
                      <select class="form-control" id="bank" name="bank">
						...
						<option value="<?=$bank_result->id?>" <?php if(!empty($single) && $single->bank_id == $bank_result->id){?>selected="selected"<?php }else if(empty($single) && !empty($fees_details) && $fees_details->bank_id == $bank_result->id){?>selected<?php }?>><?=$bank_result->bank_name?> | <?=$bank_result->account_number?></option>
						<?php }}?>
					  </select>
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" autocomplete="off" class="form-control" id="transaction_id" name="transaction_id" placeholder="Transaction Number" value="<?php if(!empty($single)){ echo $single->payment_id;}else{ echo $stu_data['transaction_id'];}?>">
					...
					<div class="col-md-3">
					 <div class="form-group">
                      ...
                      <select class="form-control" id="payment_mode" name="payment_mode">
						...
						<option value="1" <?php if(!empty($single) && $single->payment_mode == "1"){?>selected="selected"<?php }else if(empty($single) && !empty($fees_details) && $fees_details->payment_mode == '1'){?>selected<?php }?>>Online</option>
						<option value="2" <?php if(!empty($single) && $single->payment_mode == "2"){?>selected="selected"<?php }else if(empty($single) && !empty($fees_details) && $fees_details->payment_mode == '2'){?>selected<?php }?>>Challan</option>
						<option value="3" <?php if(!empty($single) && $single->payment_mode == "3"){?>selected="selected"<?php }else if(empty($single) && !empty($fees_details) && $fees_details->payment_mode == '3'){?>selected<?php }?>>Cash</option>
					  </select>
					 </div>
					 </div>
					 <div class="col-md-3">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control datepicker" id="payment_date" name="payment_date" placeholder="Payment Date" value="<?php if(!empty($single)){ echo date("d/m/Y",strtotime($single->payment_date));}else if(!empty($fees_details)){ echo date("d/m/Y",strtotime($fees_details->payment_date));}?>">
					 </div>
					 </div>
					 <div class="col-md-3">
                    <div class="form-group">
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="remark" name="remark" placeholder="Remark" value="<?php if(!empty($single)){ echo $single->remark;}else if(!empty($fees_details)){ echo $fees_details->remark;}?>">
					 </div>
					 </div>
