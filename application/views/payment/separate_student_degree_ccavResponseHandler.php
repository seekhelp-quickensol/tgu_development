<?php include('Crypto.php');
	...
	$this->db->where('id',$order_id);
	$real_fees =$this->db->get('tbl_separate_student_degree');
...
for($i = 0; $i < $dataSize; $i++) 
	...
	$this->db->select('tbl_separate_student_degree.*, tbl_separate_student.student_name, tbl_separate_student.mobile, tbl_separate_student.address');
	$this->db->where('tbl_separate_student_degree.id',$order_id);
...
<body>
	<div class="container">
		...
		<a href="<?=base_url()?>s-student-degree" style="text-decoration: none;margin-top:10px;background:#000;color:#fff;padding:10px 21px;border-radius:3px;border:none;cursor:pointer;font-size: 13px;">Back</a>
		<?php }?>
	</div>
<script>
	function myFunction() {
