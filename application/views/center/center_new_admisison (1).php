	...
	<div class="main-panel">
										...
										<div class="form-group">
											...
											<select id="course" name="course" class="form-control js-example-basic-single">
												...
												?>
														...
														<option value="<?= $course_result->id; ?>" value="<?= $course_result->id; ?>"><?= $course_result->course_name; ?></option>
		...
		<script>
			...
			$("#course").change(function() {
				...
				$("#session").empty('');
				if ($(this).val() == "23") {
					<?php if ($this->session->userdata('center_id') == "71") { ?>
						$("#session").append('<option value="6">October 2023</option>');
						$("#session").append('<option value="4">June 2024</option>');
						$("#session").append('<option value="1">January 2024</option>');
					<?php } else { ?>
						$("#session").append('<option value="4">June 2024</option>');
						$("#session").append('<option value="1">January 2024</option>');
					<?php
			...
			function final_calculation() {
				if ($("#session").val() != "" && $("#course").val() != "" && $("#stream").val() != "" && $("#country").val() != "") {
					$.ajax({
						type: "POST",
