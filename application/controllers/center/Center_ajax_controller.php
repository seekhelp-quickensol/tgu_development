...
class Center_ajax_controller extends CI_Controller
{
	...
	public function get_all_added_subject()
	{
		...
		if (!empty($document)) {
			...
			foreach ($document as $print) {
				...
				if($this->session->userdata('center_id') == '1' || $this->session->userdata('center_id') == '12'){
				       $sub_array[] = "<a data-toggle='tooltip' title='Update Subject' href=" . base_url() . "update_center_subject/" . $print->id . " target='_blank' class='btn btn-success btn-sm'><i class='mdi mdi mdi-pencil'></i></a>";
				    //   $sub_array[] = "<button title='Update Subject' class='btn btn-success btn-sm' onclick='openSubjectPopup(".$print->id.")'><i class='mdi mdi mdi-pencil'></i></button>";
				}
				$data[] = $sub_array;
			}
		}
	...
	public function get_stream_name_course(){
		$this->Center_details_model->get_stream_name_course();
	}
}