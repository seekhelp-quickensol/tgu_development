...
class Exam_controller extends CI_Controller
{
	...
	public function create_time_table()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->load->view('admin/create_time_table', $data);
		} else {
			$result = $this->Exam_model->set_time_table();
			$this->session->set_flashdata('success', 'Time table added');
			redirect('create_time_table');
		}
	}
	public function time_sheet_list()
	{
		$data['course'] = $this->Course_model->get_all_course_stream_relation();
		$data['time_sheet'] = $this->Exam_model->get_searched_timesheet();
		//echo "<pre>";print_r($data['time_sheet']);exit;
		$this->load->view('admin/time_sheet_list', $data);
	}
	...
	public function manage_admin_result()
	{
		...
		if ($this->input->post('show_subject') == "Show Subject") {
			...
			// echo "<pre>";print_r($data['exam_form']);exit;
			$data['student_exam_status'] = $this->Exam_model->get_exam_status();
			// echo "<pre>";print_r($data['student_exam_status']);exit;
	...
	public function search_admin_result()
	{
		...
		if ($this->input->post('activate_separate_marksheet_bulk') == "Activate Marksheet") {
			...
			$this->session->set_flashdata('success', 'Marksheet activated successfully');
		...
		if ($this->input->post('hold_separate_marksheet_bulk') == "Hold Marksheet") {
			...
			$this->session->set_flashdata('success', 'Marksheet inactivate successfully');
			redirect('search_admin_result');
		}
		if ($this->input->post('activate_result') == "Activate Results") {
			$this->Exam_model->activate_selected_result();
			$this->session->set_flashdata('success', 'Result activated successfully');
			redirect('search_admin_result');
		}
		if ($this->input->post('hold_result') == "Hold Results") {
			$this->Exam_model->hold_selected_result();
			$this->session->set_flashdata('success', 'Result inativated successfully');
			redirect('search_admin_result');
		}
		if ($this->input->post('delete_result') == "Delete Results") {
			$this->Exam_model->delete_selected_result();
			$this->session->set_flashdata('success', 'Result deleted successfully');
			redirect('search_admin_result');
		}
		$data['course'] = $this->Web_model->get_all_course_stream_relation();
	...
	public function search_admin_result_subject_count()
	{
		$data['result'] = array();
		if ($this->input->post('search') == "Search") {
			$data['result'] = $this->Exam_model->get_search_result_subject_count();
		}
		$data['course'] = $this->Web_model->get_all_course_stream_relation();
		$data['center'] = $this->Exam_model->get_active_center_list();
		$this->load->view('admin/search_admin_result_subject_count', $data);
	}
	public function bulk_activate_marksheet()
	{
		$data['result'] = array();
		if ($this->input->post('search') == "Search") {
			$data['result'] = $this->Exam_model->get_search_result();
			$data['exam_setup'] = $this->Exam_model->get_exam_setup($this->input->post('month'), $this->input->post('year'));
		...
		if ($this->input->post('activate_separate_marksheet_bulk') == "Activate Marksheet") {
			$this->Exam_model->bulk_marksheet_activation();
			$this->session->set_flashdata('success', 'Marksheet activated successfully');
			redirect('bulk_activate_marksheet');
		}
		$data['course'] = $this->Web_model->get_all_course_stream_relation();
		$data['center'] = $this->Exam_model->get_active_center_list();
		// $data['signature'] = $this->Setting_model->get_all_signature();
		$this->load->view('admin/bulk_activate_marksheet', $data);
	}
	public function inactivate_results()
	{
		$this->Exam_model->inactivate_results();
		$this->session->set_flashdata('success', 'Result inactivated successfully');
		redirect($_SERVER['HTTP_REFERER']);
	}
	...
	public function generate_marksheet()
	{
		...
		$data['relation_course_mode'] = $this->Exam_model->get_course_mode_relation();
		// echo "<pre>";print_r($data['course_mode']);exit;
		// $data['marksheet_signature'] = $this->Exam_model->get_single_marksheet_data();
		$data['signature'] = $this->Setting_model->get_all_signature();
		...
		if ($this->form_validation->run() === FALSE) {
			...
			if ($result == "0") {
				$this->session->set_flashdata('success', 'Marksheet generated successfully!');
			} else {
	...
	public function search_marksheet()
	{
		...
		if ($this->input->post('send_btn') == "send_btn") {
			$this->Exam_model->set_for_print_marksheet();
	...
	public function duplicate_marksheet()
	{
		...
		if ($this->input->post('send_btn') == "send_btn") {
			$this->Exam_model->set_for_print_marksheet_duplicate();
			$this->session->set_flashdata('success', 'Duplicate Marksheet successfully send for print');
			redirect('duplicate_marksheet');
		}
		$data['course'] = $this->Web_model->get_all_course_stream_relation();
	...
	public function delete_marksheet()
	{
		...
		$this->session->set_flashdata('success', 'Marksheet deleted successfully!');
		redirect($_SERVER['HTTP_REFERER']);
	}
	public function edit_marksheet()
	{
		$data['marksheet'] = $this->Exam_model->get_single_marksheet($this->uri->segment(2));
		$data['signature'] = $this->Setting_model->get_all_signature();
		$this->form_validation->set_rules('result_declare_date', 'result date', 'required');
		if ($this->form_validation->run() === FALSE) {
...
// 		echo "<pre>";print_r($data['marksheet']);exit;
	...
	public function inactive_hallticket()
	{
		...
		$this->session->set_flashdata('success', 'Hallticket inactivated successfully!');
		redirect('examination_form_list');
	}
	public function active_hallticket()
	{
		$result = $this->Exam_model->active_hallticket();
		$this->session->set_flashdata('success', 'Hallticket activated successfully!');
		redirect('examination_form_list');
	}
	...
	public function view_re_appear_subjects(){
		$this->form_validation->set_rules('id', 'name', 'required');
		if ($this->form_validation->run() === FALSE) {
			$data['single'] = $this->Exam_model->get_re_appear_exam_form_single();
			$data['subjects'] = $this->Exam_model->get_re_appear_exam_form_subject();
			$this->load->view('admin/view_re_appear_subjects', $data);
		} else {
			$this->Exam_model->update_re_appear_exam();
			$this->session->set_flashdata('success', 'Details updated successfully!');
			redirect('re_appear_examination_form_list');
	...
	public function update_exam_payment()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			redirect('examination_form_list_failed');
		}
	}
	...
	public function activate_all_results()
	{
		...
		$this->session->set_flashdata('success', 'Results Activated successfully!');
		redirect("search_admin_result");
	}
	...
	public function create_subject_quiz()
	{
		if ($this->input->post('save_single') == "save_single") {
			...
			// echo "<pre>";print_r($this->input->post('exam_question'));exit;
			$text_data = array(
				"question" 		=> $this->input->post('exam_question'),
				"selected_ans" 	=> $this->input->post('corrent_answer'),
				"options" 		=> $options,
				"img_data" 		=> '',
			);
			// echo "<pre>";print_r($text_data);exit;
			$data = array(
		...
		if ($this->form_validation->run() === FALSE) {
			// $data['course'] = $this->Web_model->get_all_course_stream_relation();
			$courseId = 23;
			$data['single_stream'] = $this->Web_model->get_all_stream_relation($courseId);
	...
	public function course_work_exam()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$courseId = 23;
			$data['course'] = $this->Web_model->get_all_stream_relation($courseId);
			$data['question'] = $this->Exam_model->get_phd_subject_question();
		...
		if ($this->form_validation->run() === FALSE) {
			...
			if ($this->input->post('upload') == "Upload") {
				...
				if (isset($_FILES["userfile"]["name"])) {
					...
					// echo "<pre>";print_r($data);exit;
					if (!empty($store_data)) {
						$this->Exam_model->insert_batch_course_work_question($store_data);
						$this->session->set_flashdata('success', 'Questions added successfully');
					}
				}
			}
	...
	{
		if ($this->input->post('send_btn') == "send_btn") {
			$this->Exam_model->sent_coursework_for_print();
	...
	public function print_course_work_marksheet()
	{
		...
		$data['subject_details'] = $this->Exam_model->get_course_work_marksheet_details($this->uri->segment(2));
		$data['signature'] = $this->Setting_model->get_all_signature();
	...
	public function view_all_paper()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			if ($res == 0) {
				...
				$this->session->set_flashdata('success', 'Papers updated successfully!!');
			}
			redirect('add_paper');
			// redirect('view_all_paper/' . $this->uri->segment(2));
		}
	}
	...
	public function print_course_work_marksheet_separate_student()
	{
		...
		$data['subject_details'] = $this->Exam_model->get_course_work_marksheet_details_separate_student($this->uri->segment(2));
		$this->load->view('admin/SimpleFunctions');
		// $this->load->view('admin/print_course_work_marksheet',$data);
	...
	public function admin_fill_exam_form()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			if (!empty($data['student'])) {
				if (!empty($data['student']->id) && !empty($data['student']->year_sem)) {
					$exam_form_check = $this->Exam_model->get_exam_form_check_new($data['student']->id, $data['student']->year_sem);
			...
			$exam_form_check = $this->Exam_model->get_exam_form_check();
			if (!empty($exam_form_check)) {
				$this->session->set_flashdata('message', 'You have already filled exam form');
				redirect('admin_fill_exam_form');
			} else {
	...
	public function student_examination_history()
	{
		$this->form_validation->set_rules('enrollment_number', 'enrollment number', 'required');
		if ($this->form_validation->run() === FALSE) {
			$this->load->view('admin/student_examination_history');
		} else {
			$this->Exam_model->check_student_examination_history();
			$this->session->set_flashdata('message', 'Please enter correct enrollment number!');
			redirect('student_examination_history');
		}
	}
	public function student_examination_history_activity()
	{
		$data['activity'] = $this->Exam_model->get_student_all_examination_activities();
		// echo "<pre>";print_r($data['activity']);exit;
		$data['student'] = $this->Student_certificate_model->get_single_student_info();
		$this->load->view('admin/student_examination_history_activity', $data);
	}
	public function marksheet_back_sample()
	{
		$this->load->view('marksheet/marksheet_back_sample');
	}
}