<?php include('header.php');
	// if(!empty($no_split) && $no_split->payment_status != '1'){
	// 	redirect('pay_no_split_fees');
	// }		
?>
<style>
@page {
    ...
    <div class="container">
        <div class="row">
            <div class="layer_data">
                <div class="col-md-12">
                    <h2 class="mb-3">No Split Issue Letter</h2>
							    ...
							    <?php if(!empty($student) && $student->admission_status == '4'){?> 
    								<?php if(empty($no_split)){ ?> 
    										<div class="box_layer_t"> 
    											...
    											<p class="mid_bix">Dear <b><?=$student->student_name?></b>, please complete No Split Issue Letter payment/fees to get your Letter.</p>
    										<a style="text-align:center" href="<?=base_url("pay_no_split_fees");?>" class="btn btn-success">Pay No Split Issue Letter fees</a> 
    									</div>
    								<?php }else{ ?>  
    									<?php if($no_split->approve_status == '0'){ ?> 
    										<div class="box_layer_t">
    										                <div class="btn_center"> 
    														    <div class="step_Box">
    														        <ul>
    												...
    												<p class="mid_bix">Dear <b><?=$student->student_name?></b>, Please be patient your No Split Issue Letter is under process, once it will approved your No Split Issue Letter will be availble here.</p>
    										    </div> 
    											
    											<?php }elseif($no_split->approve_status == "1"){ ?>  
    										 <div class="box_layer_t">
    										                <div class="btn_center"> 
    														    <div class="step_Box">
    														        <ul>
    												    ...
    												    <p class="mid_bix">Congratulations! your No Split Issue Letter has been approved, please <a href="<?=base_url();?>print_no_split/<?=base64_encode($no_split->id);?>" target="_blank">Click here</a> to print your No Split Issue Letter.</p>
    										    </div> 
    										<?php } ?> 
    								<?php } ?>
    							<?php }else{?>
            					    <div class="box_layer_t nodata_img d-flex justify-content-center" style="background-image:url('<?=base_url();?>images/nodata.jpg')">    
            					        <div class="btn_center"> 
        									<div class="step_Box">
        										<p>Sorry! You are not eligible to apply for No Split Issue Letter.</p>
        									</div>
        								</div> 
        							</div>
        					    <?php }?>
