...
class Center_controller extends CI_Controller{
	...
	public function center_student_qualification()
	{
		if ($this->input->post('upload_photo') == "Upload") {
			...
			$this->Center_details_model->update_student_identity_soft($photo, $identity_file, $signature);
	...
	public function center_manage_result()
	{
		...
		if ($this->input->post('show_subject') == "Show Subject") {
			...
			if (!empty($data['course_stream'])) {
				if($data['course_stream']->year_sem >1){
					$re_registration = $this->Center_details_model->check_re_registration_before_result($data['course_stream']->id,$data['course_stream']->year_sem,$data['course_stream']->admission_type,$data['course_stream']->lateral_year);
