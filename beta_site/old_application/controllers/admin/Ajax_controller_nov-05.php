	...
	public function get_all_admission_list(){
        ...
        if(!empty($document)){
			...
			foreach($document as $print){
				...
				$noc = "";
				if($print->noc != ''){
					$noc = "<a target='_blank' data-toggle='tooltip' title='NOC' href=" . base_url() . "images/noc/" . $print->noc . " class='btn btn-danger btn-sm delete_class'>NOC</a>";
				} 
	...
	public function get_refunded_phd_registration(){
        ...
        if(!empty($document)){
			...
			foreach($document as $print){
				...
				// $refund = $this->db->get('tbl_research_refund');
				// $refund = $refund->num_rows();  				
				// $refund_arr= "<a  data-toggle='tooltip' title='Refund' href=" . base_url() . "add_refund_research/" . $print->id . " class='btn btn-danger btn-sm delete_class'>Refund</a>";
