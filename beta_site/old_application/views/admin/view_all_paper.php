        ...
        <div class="content-wrapper">
		  ...
		  <div class="col-md-6 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  ...
                  <form class="forms-sample" name="paper_form" id="paper_form" method="post" enctype="multipart/form-data">
					...
					<div class="form-group">
                      <label for="exampleInputUsername1">Upload File *
							&nbsp;&nbsp;
					  <?php if(!empty($single) && $single->file != ""){?>
						<a href="<?=$this->Digitalocean_model->get_photo('uploads/papers/'.$single->file)?>" target="_blank">View</a>
					<?php }?>
            ...
            <div class="col-md-6 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">  
                  <h6 class="card-title">Course Name - <?php if(!empty($single)){ echo $single->course_name;}?>  Stream Name - <?php if(!empty($single)){ echo  $single->stream_name;}?></h6>
            
				  <a href="<?=base_url();?>add_paper" class="btn btn-primary" style="position: absolute; right: 15px; top: 25px;">Back</a>
				  <br><br>
                  <p class="card-description">
                    All list of Papers
                  </p>
...
$(document).ready(function() { 
	...
	var table = $('#example').DataTable({
		...
		"ajax":{
			"url" : "<?=base_url();?>admin/Ajax_controller/get_all_paper_ajax_course_wise",
			"type": "POST",
            "data":{"stream_id":<?php echo $this->uri->segment(2);?>},
		},	
		"complete": function() {
...
$(document).ready(function() {
	$(".course_relation").on("keyup change", function() {  
		if($("#course").val() != '' && $("#stream").val() != '' && $("#subject").val() != '' && $("#course_mode").val() != ''){
			$.ajax({
				type: "POST",
				url: "<?=base_url();?>admin/Ajax_controller/get_unique_course_relation",
				data:{'course':$("#course").val(),'stream':$("#stream").val(),'subject':$("#subject").val(),'course_mode':$("#course_mode").val(),'id':'<?=$id?>'},
				success: function(data){
					console.log(data);
					if(data == "0"){
						$("#relation_error").html('');
						...
						$("#course_mode_error").html('This relation is already added');
						$("#submit").hide();
					}  
				},
				error: function(jqXHR, textStatus, errorThrown) {
