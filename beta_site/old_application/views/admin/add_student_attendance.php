							...
							<div class="card-body">
								...
								<table class="table table-striped table-bordered dt-responsive nowrap" style="width:100%"> 
									<tr>
										...
										?>
										<td><input type="checkbox" name="student_name[]" value="<?=$student_result->id?>" <?php if(!empty($explode) && in_array($student_result->id,$explode)){?>checked="checked"<?php }?>><?=$student_result->student_name?></td>
										<?php }?>
									</tr>   
								</table>
							</div>
...
$("#date").change(function(){
	location.href = "<?=base_url();?>add_student_attendance/<?=$this->uri->segment(2)?>/"+$("#date").val();
}); 
$(function(){
