        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                   <h4 class="card-title">Success Re Appear List</h4><p class="card-description">
				...
				<tbody>
					...
					if(!empty($form)){ foreach($form as $form_result){?>
						<tr>
							<td><?=$sr++?></td>
							<!--<td><input type="checkbox" name="ids[]" value="<?=$form_result->id?>"></td>-->
							 
							<td>
								<?php if($form_result->hall_status == '0'){?>
								<a onclick="return confirm('Are you sure, you want to activate this record?')" type="button" title="Activate Hallticket" data-toggle="tooltip" href="<?=base_url();?>active_re_appear_hallticket/<?=$form_result->id?>" class="btn btn-primary btn-sm"><i class="mdi mdi-bookmark-check"></i></a> 
								<?php }else{?>
								<a onclick="return confirm('Are you sure, you want to inactivate this record?')" type="button" title="In Activate Hallticket" data-toggle="tooltip" href="<?=base_url();?>inactive_re_appear_hallticket/<?=$form_result->id?>" class="btn btn-danger btn-sm"><i class="mdi mdi-bookmark-check"></i></a> 
								<?php }?>
