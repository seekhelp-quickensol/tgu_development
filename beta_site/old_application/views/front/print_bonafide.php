					...
					<div>
						<div class="letter_area" style="background-image:url(<?=$this->Digitalocean_model->get_photo('images/new_marksheet-without.jpeg')?>);background-position: center;background-size: cover;border: 2px solid #e8612b;height: 100%;">  
            							...
            							<div class="head_top_logo"><img src="<?=$this->Digitalocean_model->get_photo('images/only-logo.png')?>"></div>
                                        	...
                                        	<p style="float: left;font-size: 15px;color: #000;font-weight: 700;">Ref No.: BTU/<?=$single->id;?>/Bonafide/<?=date('Y', strtotime($single->application_date));?></p>
                                            </div>
							                <div style="clear:both"></div>
							...
							<div style="display: block;">
									...
									<p style="margin-top: 10px;color: #000;font-size: 18px;font-weight: 500;margin-bottom: 0px;line-height: 40px; word-spacing: 7px;float:left">
                                        ...
                                        <?php }else{ ?>
                                            D/O 
                                        <?php } ?>
                                        <b><?php if(!empty($single)){echo $single->father_name;}?></b> , Enrollment No . <b><?php echo $single->enrollment_no;?></b>, was a bonafide Regular student of Bir Tikendrajit University. 
                                        <?php if($single->gender == 'Male'){?>
                                            He
                                        <?php }else{ ?>
                                            She
                                        <?php } ?>
                                        has successfully completed the <b><?php echo $course_name;?> (<?php echo $stream_name;?>)</b> Degree Programme in <b><?=$division["date"];?></b> from our University.
									</p>
									<p style="margin-top: 10px;color: #000;font-size: 18px;font-weight: 500;margin-bottom: 0px;line-height: 40px; word-spacing: 7px;float:left">
