    ...
    <div class="container"> 
        <div class="edu_tabs" style="text-align:center">
            <?php if(!empty($mail)){ echo $mail->content;}else{ 
			?>
			<style>body {
...
<!--   <div class="light">
    ...
    <span></span>
			...
			<?php }?>
			<br>
			<?php if(!empty($mail) && $mail->file !=""){ 
				$exp = explode(",",$mail->file);
				for($i=0;$i<count($exp)-1;$i++){
			?>
			<p><a href="<?=$this->Digitalocean_model->get_photo('images/mail_file/'.$exp[$i])?>">Click to View File</a></p>
			<?php }}?>
