<?php include('Crypto.php');
	...
	$exist = $this->db->get('tbl_separate_transcript');
	...
	if($this->session->userdata('degree_order_id') != $order_id){
		...
		$real_fees = $this->db->get('tbl_separate_transcript');
...

	{
		...
		$information=explode('=',$decryptValues[$i]);
	    	...
	    	echo '<tr><tBIR TIKENDRAJIT UNIVERSITY.'</td><td>'.urldecode($information[1]).'</td></tr>';
...
for($i = 0; $i < $dataSize; $i++) 
	...
	$this->db->select('tbl_separate_transcript.*, tbl_separate_student.student_name,tbl_separate_student.mobile,tbl_separate_student.address');

	$this->db->where('tbl_separate_transcript.registration_id',$order_id);
	...
	$data = array(
		...
		'separate_student_id' => $result->registration_id
...
<body>
	...
	<div class="container">
		...
		<a href="<?=base_url()?>transcript_application" style="text-decoration: none;margin-top:10px;background:#000;color:#fff;padding:10px 21px;border-radius:3px;border:none;cursor:pointer;font-size: 13px;">Back</a>
