        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-md-6 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Blogs Setting</h4>
                  <p class="card-description">
                    Please enter blogs details
                  </p>
                  <form class="forms-sample" name="news_form" id="news_form" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="link" name="link" placeholder="Link" value="<?php if(!empty($single)){ echo $single->link;}?>">
        
                    </div>
					<div class="form-group">
                      <label for="exampleInputUsername1">Published Date*</label>
                      <input data-provide="datepicker" type="text" class="form-control datepicker" id="date" name="date" placeholder="Date" value="<?php if(!empty($single)){ echo date("d-m-Y",strtotime($single->date));}?>" autocomplete="off">
                    </div>
					          <div class="form-group">
                      <label for="exampleInputUsername1">Short Description*</label>
                      <textarea class="form-control" id="content" name="content" placeholder="Short Description"><?php if(!empty($single)){ echo $single->blogs;}?></textarea>
                    </div>

                    <div class="form-group">
                      <label for="exampleInputUsername1">Long Description*</label>
                      <textarea class="form-control" id="long_content" name="long_content" placeholder="Long Description"><?php if(!empty($single)){ echo $single->long_blogs;}?></textarea>
                    </div>
                    <div class="form-group">
                      <label>Image*</label>
...
$(".datepicker").datepicker({
 ...
 $(document).ready(function () {		
	...
	$('#news_form').validate({
    ignore: ":hidden:not(#long_content),.note-editable.panel-body",
		rules: {
			heading: {
				required: true,
			},
		...
		messages: {
			...
			content: {
				required: "Please enter short description of blog",
            },
            long_content: {
                required: "Please enter long description of blog",
            },
            photo:{
                required: "Please upload image of blog",
            },
		},
		errorElement: 'span',
...
$(document).ready(function() { 
	...
	var table = $('#example').DataTable({
		...
		"ajax":{
			"url" : "<?=base_url();?>admin/Ajax_controller/get_all_blogs_ajax",
			"type": "POST",
		},	
		"complete": function() {
