...
?>
		...
		<div class="book letter_section" id="letter_section">
			<div class="page">
				<div class="subpage"  style="background-image:url(<?=$this->Digitalocean_model->get_photo('images/250-350.png')?>)">
					...
					<div class="print-wrapper">
						<?php  
							$smart = $marksheet_id->id."-".$marksheet->id."-".$marksheet->enrollment."-".$marksheet->student_name."-".$marksheet->father_name."-".$marksheet->created_on;
							$barcode = $marksheet->enrollment."-".$marksheet_id->id;
							$last_exam = $this->Consolidated_model->get_last_exam_year_separate($marksheet->id);
							?>
						<div class="first-row">
							<div class="scan-img">
						...
						<div class="markshit-header ">
							<h2 style="font-size:14px;color: #335c9e;margin-bottom: 6px; margin-top: 92px;"><?php if(!empty($university_details)){ echo $university_details->address;}?></h2>
						...
						<div class="students-info student-info-right" style="height:35px;">
						  ...
						  <p>Enrollment .No. : <?=$marksheet->enrollment?></p>
						</div>
						...
						<div class="row">
								...
								<?php 
									...
									$subject = $this->Consolidated_model->get_selected_semester_subject_for_result_separate($marksheet_id->id,$i);
							...
							<table class="students-exam-details" style="margin-top: 44px;width: 97%;">
								<body>
									...
									<tr class="student-marks-total">
										...
										<?php 
											...
											for($y=1;$y<=$marksheet->year_sem;$y++){
												$mark = $this->Consolidated_model->get_total_marks_cons_separate($y,$marksheet_id->id);	
												$total_mark += $mark;
										?>
										<td><?=$mark?></td>
										<?php }?> 	
									...
									<tr class="student-marks-total">
										<td>Mark Obtained</td>
										<?php for($y=1;$y<=$marksheet->year_sem;$y++){
											$mark_obtaind = $this->Consolidated_model->get_total_obt_marks_cons_separate($y,$marksheet_id->id);
			...
			<div class="right-controller">
				<div style="text-align:center;">
					<img style="width: 90px;display: block;margin: 0px auto;" src="<?=$this->Digitalocean_model->get_photo('certificate_signature/'.$marksheet->signature)?>">
					<strong class="marksheet_sign"><?=$marksheet->dispalay_signature;?></strong>
				</div>
			</div>
    ...
    padding-top: 5px;">
					...
					<div class="print-wrapper">
						...
						<div style="clear:both"></div>
						<div class="markshit-header ">
							<h2 style="font-size:14px;color: #335c9e;margin-bottom: 6px; margin-top: 122px;"><?php if(!empty($university_details)){ echo $university_details->address;}?></h2>
							<h2 class="statement"> Consolidated statement of marks</h2>
							<h3><?=$marksheet->course_name?> (<?=$marksheet->stream_name?>)</h3>
						</div>
						<div class="students-info student-info-left" style="margin-bottom: 0px;">
			...
			<div class="right-controller">
				<div style="text-align:center;">
					<img style="width: 90px;display: block;margin: 0px auto;" src="<?=$this->Digitalocean_model->get_photo('certificate_signature/'.$marksheet_id->signature)?>">
					<strong class="marksheet_sign"><?=$marksheet_id->dispalay_signature;?></strong>
				</div>
			</div>
