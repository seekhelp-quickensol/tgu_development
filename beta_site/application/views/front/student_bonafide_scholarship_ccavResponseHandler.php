...
$order_id = $order_id[1];
	...
	$this->db->where('id !=',$order_id); 
	$exist = $this->db->get('tbl_bonafide_cer_application_scholarship'); 
	$exist = $exist->row(); 
	if(!empty($exist)){  
	...
	$this->db->where('id',$order_id); 
	$real_fees =$this->db->get('tbl_bonafide_cer_application_scholarship'); 
...
$this->db->where('tbl_bonafide_cer_application_scholarship.id',$order_id);
