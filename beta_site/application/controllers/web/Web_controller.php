...
// 			echo "<pre>";print_r($data['students']);exit;
	...
	public function agreement_document_status(){
		if($this->input->post('agreement_status') == 'Skip'){
			$center_id = $this->input->post('center_id');
			$paument_id = $this->input->post('payment_id');
			redirect('make_center_payment/' . base64_encode($center_id) . '/' . base64_encode($paument_id));
		}else if($this->input->post('agreement_status') == 'Yes'){
			$center_id = $this->input->post('center_id');
			$paument_id = $this->input->post('payment_id');
			redirect('download_agreement_document/' . base64_encode($center_id) . '/' . base64_encode($paument_id));
		}
		$this->load->view('front/agreement_document_status');
	}
	public function download_agreement_document(){
		if($this->input->post('agreement_status') == 'Continue'){
			$center_id = $this->input->post('center_id');
			$paument_id = $this->input->post('payment_id');
			redirect('make_center_payment/' . base64_encode($center_id) . '/' . base64_encode($paument_id));
		}
		$this->load->view('front/download_agreement_document');
	}
}