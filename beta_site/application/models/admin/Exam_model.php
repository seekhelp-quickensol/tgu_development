...
class Exam_model extends CI_Model { 
	...
	public function get_re_registered_student_year_sem($enrollment_number,$current_year_sem){ 
		...
		if($student->admission_type == "0"){
			...
			$this->db->where('enrollment_number',$enrollment_number);
			$this->db->where('current_year_sem',$current_year_sem);
	...
	public function sent_coursework_for_print(){ 
	...
	public function set_for_print_marksheet(){ 
		$ids = $this->input->post('ids'); 
		if(!empty($ids)){
			for($i=0;$i<count($ids);$i++){
				$data = array(
	...
	public function set_for_print_marksheet_duplicate(){ 
		$ids = $this->input->post('print_id'); 
		if(!empty($ids)){
			for($i=0;$i<count($ids);$i++){
				$data = array(
					'sent_for_print' => '1',
				...
				$this->db->where('id',$ids[$i]);
				$this->db->update('tbl_duplicate_marksheet',$data);
			}
		}
		return true;
	}
	...
	public function send_single_password(){
		...
		$result = $result->row();
		if(!empty($result)){
			$message = "Hello ".$result->student_name.",<br><br> I hope this email finds you well. Below is the Login Detail for Ph.D examination.<br>";
			$message .= "URL: ".base_url()."phd_exam_login <br>";
			$message .="Username: ".$result->mobile_number." <br>";
			...
			$to = array(
        		...
        		"name" => $this->security->xss_clean($result->student_name),
        	);
        	$subject = 'Login Details | '.no_reply_name;    
	...
	public function get_result_subject(){
		...
		//echo $student->course_mode;exit;
		if(!empty($student)){
			$this->db->where('tbl_subject.is_deleted','0');
			$this->db->where('tbl_subject.status','1'); 
			//$this->db->where_in('tbl_subject.is_center',array('0','6')); 
			$this->db->where_in('tbl_subject.center_id',array('1','6')); 
	...
	public function check_result_exists($month,$year,$exam_status,$year_sem,$enrollment){
		$student = $this->get_result_student();
		if(!empty($student)){	
			$this->db->where('examination_month', $month);
			$this->db->where('examination_year', $year);
			$this->db->where('examination_status', $exam_status);
	...
	public function set_result(){
		...
		/*	$query = @unserialize(file_get_contents('http://ip-api.com/php/'.$ip));
			...
			// 	$result_declare_date = "2023-07-25";
			// }
			// if($this->input->post('txtMonth') == "April" && $this->input->post('txtYear') == "2023"){
			// 	$result_declare_date = "2023-07-25";
			// }
			// if($this->input->post('txtMonth') == "August" && $this->input->post('txtYear') == "2023"){
			// 	$result_declare_date = "2023-09-15";
			...
			// }else if($single_result->examination_month =='June' && $single_result->examination_year =='2023'){
			...
			// 	$issue_date="25-07-2023";
			// 	$result_date = "25-07-2023";
			// }else if($single_result->examination_month =='April' && $single_result->examination_year =='2023'){
			// 	$issue_date="25-07-2023";
			// 	$result_date = "25-07-2023";
			// }else if($single_result->examination_month =='August' && $single_result->examination_year =='2023'){
			// 	$issue_date="15-09-2023";
			// 	$result_date = "15-09-2023";
			// }else if($single_result->examination_month =='December' && $single_result->examination_year =='2022'){
			// 	$issue_date="10-01-2023";
			// 	$result_date = "10-01-2023";
			// }else if($single_result->examination_month =='September' && $single_result->examination_year =='2023'){
			// 	$issue_date="10-10-2023";
			// 	$result_date = "10-10-2023";
			// }else if($single_result->examination_month =='March' && $single_result->examination_year =='2023'){
			// 	$issue_date="25-07-2023";
			// 	$result_date = "25-07-2023";
			// }
			$data = array(
			...
			if($this->security->xss_clean($this->input->post('comboTResult')) == '0'){
				...
				$this->email->subject("Information from Global");
	...
	public function get_search_result_subject_count(){ 
		$this->db->select('tbl_exam_results.*,tbl_course.course_name,tbl_stream.stream_name,tbl_student.student_name,tbl_student.course_mode');
		if($this->input->post('month') != ""){
			$this->db->where('tbl_exam_results.examination_month',$this->input->post('month'));
		}
		...
		if(!empty($result)){
			foreach($result as $result_result){
				$this->db->where('is_deleted','0');
				$this->db->where('result_id',$result_result->id);
				$total_subject = $this->db->get('tbl_examination_result_details');
				$total_subject = $total_subject->num_rows();
				$result[$i]->total_subject = $total_subject;
				$i++;
			}
		}
		return $result;
	...
	public function get_course_marksheet_student(){
		$this->db->select('tbl_student.*,tbl_course.course_name,tbl_stream.stream_name');
		$this->db->where('tbl_student.is_deleted','0');
	...
	public function get_course_mode_relation(){
		$course_mode = $this->get_course_marksheet_student();
		// echo "<pre>";print_r($course_mode);exit;
		$this->db->select('tbl_course_stream_relation.*');
		$this->db->where('tbl_course_stream_relation.course',$course_mode->course_id);
		$this->db->where('tbl_course_stream_relation.stream',$course_mode->stream_id);
		$this->db->where('tbl_course_stream_relation.course_mode',$course_mode->course_mode);
		$result = $this->db->get('tbl_course_stream_relation');
		return $result->row();
		// $result = $result->row();
		// echo "<pre>";print_r($course_mode);exit;
	...
	public function delete_result(){
		...
		$this->db->where('id',$this->uri->segment(2));
		$this->db->where('id',$this->uri->segment(2));
		$exam_data = $this->db->get('tbl_exam_results');
		$exam_data = $exam_data->row();
		
		$this->db->where('id',$exam_data->student_id);
		$student_row = $this->db->get('tbl_student');
		$student_row = $student_row->row();
		$profile = $this->Admin_model->get_profile();
		$add_log = 1;
		$log_description = $profile->first_name." ".$profile->last_name." has deleted exam result of ".$student_row->student_name." (".$student_row->id.")(".$exam_data->examination_month." ".$exam_data->examination_year."->".$exam_data->year_sem.")". " on ".date("d/m/Y");
		$log = array(
	...
	public function generate_marksheet(){ 
		if($this->input->post('marksheet_number') == ""){
			...
			if($this->input->post('final_status') == "1"){
				...
				$this->db->update('tbl_student',$stude_data);
			}
			return 0;
		}else{
			$data = array(
	...
	public function get_search_markheet(){
		...
		if($this->input->post('exam_month') != ""){
			$this->db->where('tbl_exam_results.examination_month',$this->input->post('exam_month'));
		} 
		if($this->input->post('exam_year') != ""){
			$this->db->where('tbl_exam_results.examination_year',$this->input->post('exam_year'));
		} 
	...
	public function delete_marksheet(){
		...
		$this->db->where('id',$this->uri->segment(2));
		$exam_data = $this->db->get('tbl_marksheet');
		$exam_data = $exam_data->row();
		...
		$profile = $this->Admin_model->get_profile();
		$add_log = 1;
		$log_description = $profile->first_name." ".$profile->last_name." has deleted marksheet of ".$student_row->student_name." (".$student_row->id.")(".$exam_data->id."->".$exam_data->year_sem.")". " on ".date("d/m/Y");
		$log = array(
			'user_id' 		=> $this->session->userdata('admin_id'),
			'student_id' 	=> $student_row->id,
			'description' 	=> $log_description,
	...
	public function get_single_marksheet($id){
		$this->db->select('tbl_marksheet.*,tbl_signature.name as signature_name,tbl_signature.signature,tbl_signature.dispalay_signature,tbl_exam_results.result,tbl_exam_results.examination_month,tbl_exam_results.examination_year,tbl_student.mother_name,tbl_student.student_name,tbl_student.father_name,tbl_student.center_id,tbl_student.date_of_birth,tbl_student.course_mode,tbl_course.course_name,tbl_stream.stream_name,tbl_session.session_name,tbl_session.pre_session,tbl_course.course_name,tbl_course.id as course_id,prepared_tbl_signature.name as prepared_signature_name,prepared_tbl_signature.signature as prepared_signature,prepared_tbl_signature.dispalay_signature as prepared_dispalay_signature,tbl_checked_signature.name as checked_signature_name,tbl_checked_signature.signature as checked_signature,tbl_checked_signature.dispalay_signature as checked_dispalay_signature');
		$this->db->where('tbl_marksheet.is_deleted','0');
		$this->db->where('tbl_marksheet.id',$id);
		$this->db->join('tbl_exam_results','tbl_exam_results.id = tbl_marksheet.result_id');
		$this->db->join('tbl_student','tbl_student.id = tbl_marksheet.student_id');
		$this->db->join('tbl_course','tbl_course.id = tbl_student.course_id');
		$this->db->join('tbl_session','tbl_session.id = tbl_student.session_id');
		$this->db->join('tbl_stream','tbl_stream.id = tbl_student.stream_id');
		$this->db->join('tbl_signature','tbl_signature.id = tbl_marksheet.signature_id','left');
		$this->db->join("tbl_signature as prepared_tbl_signature","prepared_tbl_signature.id = tbl_marksheet.prepared_signature_id",'left');
		$this->db->join("tbl_signature as tbl_checked_signature","tbl_checked_signature.id = tbl_marksheet.checked_signature_id",'left');
		$result = $this->db->get('tbl_marksheet');
		return $result->row();
	}
	public function edit_marksheet(){
		$data = array(
			...
			'credit_transfer_remark'			=> $this->input->post('remark'),
			'signature_id'       				=> $this->input->post('signature'),
			'checked_signature_id'       		=> $this->input->post('checked_by_signature'),
			'prepared_signature_id'       		=> $this->input->post('prepared_by_signature'),
		);
		$this->db->where('id',$this->input->post('id'));
		$this->db->update('tbl_marksheet',$data);
		return true;
	...
	public function get_exam_form_remark($exam_id){
	...
	public function get_all_examination_form_list($length,$start,$search){
		...
		if($this->input->post('month') != ""){
			$this->db->where('tbl_examination_form.exam_month',$this->input->post('month'));   
			$this->db->where('tbl_examination_form.exam_year',$this->input->post('year'));   
		}
		if($search !=""){
			$this->db->group_start(); 
			$this->db->or_like('tbl_course.course_name',$search);
			...
			$this->db->or_like('tbl_examination_form.exam_month',$search);
			$this->db->or_like('tbl_examination_form.exam_year',$search);
	...
	public function get_re_appear_exam_form(){
		$this->db->select('tbl_center.center_name,tbl_re_appear.*,tbl_course.course_name,tbl_stream.stream_name,tbl_student.father_name,tbl_student.mother_name,tbl_student.student_name,tbl_student.mobile,tbl_student.email,tbl_student.enrollment_number');
		$this->db->where('tbl_re_appear.is_deleted','0'); 
		$this->db->where('tbl_re_appear.payment_status','1');  
		$this->db->join('tbl_student','tbl_student.enrollment_number = tbl_re_appear.enrollment_number');
		$this->db->join('tbl_course','tbl_course.id = tbl_student.course_id');
		$this->db->join('tbl_stream','tbl_stream.id = tbl_student.stream_id');    
		$this->db->join('tbl_center','tbl_center.id = tbl_student.center_id');         
		$this->db->order_by('tbl_re_appear.id','DESC');  
		$result = $this->db->get('tbl_re_appear');
		return $result->result();
	}
	public function get_re_appear_exam_form_failed(){
		$this->db->select('tbl_center.center_name,tbl_re_appear.*,tbl_course.course_name,tbl_stream.stream_name,tbl_student.father_name,tbl_student.mother_name,tbl_student.student_name,tbl_student.mobile,tbl_student.email,tbl_student.enrollment_number');
		$this->db->where('tbl_re_appear.is_deleted','0'); 
		$this->db->where('tbl_re_appear.payment_status','0');  
		$this->db->join('tbl_student','tbl_student.enrollment_number = tbl_re_appear.enrollment_number');
		$this->db->join('tbl_course','tbl_course.id = tbl_student.course_id');
		$this->db->join('tbl_stream','tbl_stream.id = tbl_student.stream_id');    
		$this->db->join('tbl_center','tbl_center.id = tbl_student.center_id');         
	...
	public function get_re_appear_exam_form_single(){
		$this->db->where('id',$this->uri->segment(2));
		$this->db->where('is_deleted','0');
		$result = $this->db->get('tbl_re_appear');
		return $result->row();
	}
	public function get_re_appear_exam_form_subject(){
		$this->db->select('tbl_subject.subject_code,tbl_subject.subject_name,tbl_re_appear_subjects.id');
		$this->db->where('tbl_re_appear_subjects.re_appear_id',$this->uri->segment(2));
		$this->db->where('tbl_re_appear_subjects.is_deleted','0');
		$this->db->join('tbl_subject','tbl_subject.id = tbl_re_appear_subjects.subject_id');
		$result = $this->db->get('tbl_re_appear_subjects');
		return $result->result();
	}
	public function update_re_appear_exam(){ 
		$data = array(
	...
	public function get_all_re_appear_examination_form_list_count($search){
		$this->db->select('tbl_center.center_name,tbl_re_appear.*,tbl_course.course_name,tbl_stream.stream_name,tbl_student.father_name,tbl_student.mother_name,tbl_student.student_name,tbl_student.mobile,tbl_student.email,tbl_student.enrollment_number');
		$this->db->where('tbl_re_appear.is_deleted','0');  
		$this->db->where('tbl_re_appear.payment_status','1');   
		...
		$this->db->join('tbl_stream','tbl_stream.id = tbl_student.stream_id');
		$this->db->join('tbl_center','tbl_center.id = tbl_student.center_id'); 		
		$this->db->order_by('tbl_re_appear.created_on','DESC');  
		$result = $this->db->get('tbl_re_appear');
		return $result->num_rows();
	}
	public function get_all_re_appear_examination_form_list_failed($length,$start,$search){
		$this->db->select('tbl_center.center_name,tbl_re_appear.*,tbl_course.course_name,tbl_stream.stream_name,tbl_student.father_name,tbl_student.mother_name,tbl_student.student_name,tbl_student.mobile,tbl_student.email,tbl_student.enrollment_number');
		$this->db->where('tbl_re_appear.is_deleted','0'); 
		$this->db->where('tbl_re_appear.payment_status','0');  
	...
	public function get_all_re_appear_examination_form_list_failed_count($search){
		$this->db->select('tbl_center.center_name,tbl_re_appear.*,tbl_course.course_name,tbl_stream.stream_name,tbl_student.father_name,tbl_student.mother_name,tbl_student.student_name,tbl_student.mobile,tbl_student.email,tbl_student.enrollment_number');
		$this->db->where('tbl_re_appear.is_deleted','0');  
		$this->db->where('tbl_re_appear.payment_status','0');   
	...
	public function get_all_examination_form_list_failed_count($search){
		...
		$this->db->join('tbl_session','tbl_session.id = tbl_examination_form.session');    
		$this->db->order_by('tbl_examination_form.id','DESC');  
		$this->db->group_by('tbl_examination_form.student_id,year_sem');
		$result = $this->db->get('tbl_examination_form');
		return $result->num_rows();
	}
...
// 	}
    ...
    public function generated_results_excel(){
        ...
        $this->db->where("tbl_marksheet.enrollment_number",$this->input->post('enrollment_number'));
		...
		if($this->input->post("date") != ""){
			$this->db->where("Date(tbl_marksheet.created_on) <=",$to_date);
			$this->db->where("Date(tbl_marksheet.created_on) >=",$from_date);
		}
		$this->db->join("tbl_exam_results","tbl_exam_results.id = tbl_marksheet.result_id");
		$this->db->join("tbl_student","tbl_student.id = tbl_marksheet.student_id");
		$this->db->join("tbl_course","tbl_course.id = tbl_student.course_id");
		...
		foreach ($result as $res){
			...
			$array_of_row = array();
			$array_of_row[] = $i++; 
			$array_of_row[] = "<input class='checkbox' type='checkbox' name='ids[]' value='".$res->id."'>";
			$array_of_row[] = $image;
			...
			$array_of_row[] = $res->result_declare_date;
            for($z=0;$z<20;$z++){ 
    			if(!empty($data[$z]) && $data[$z]->subject_name != ""){
	...
	public function generated_results_excel_pre(){
		$to_date = date("Y-m-d",strtotime($this->input->post("to_date"))).' 00:00:00';
	...
	public function get_all_document_verification_data($length,$start,$search){ 
		...
		$this->db->where('tbl_document_verification.is_online','0');  
		if($search !=""){
			$this->db->group_start();
		...
		$this->db->order_by('tbl_document_verification.id','DESC');
		$this->db->limit($length,$start);
	...
	public function get_all_document_verification_data_offline($length,$start,$search){ 
		if($this->input->post('type') !==""){
			$this->db->where('tbl_document_verification.student_type',$this->input->post('type'));
		}
		$this->db->where('tbl_document_verification.is_deleted','0');
		//$this->db->where('tbl_document_verification.payment_status','1'); 
		$this->db->where('tbl_document_verification.is_online','1');  
		if($search !=""){
			$this->db->group_start();
			$this->db->or_like('tbl_document_verification.name',$search);
			$this->db->or_like('tbl_document_verification.address',$search);
	...
	public function get_all_document_verification_data_offline_count($search){
		if($this->input->post('type') !==""){
			$this->db->where('tbl_document_verification.student_type',$this->input->post('type'));
		}
		$this->db->where('tbl_document_verification.is_deleted','0');
	...
	public function get_single_offline_document_verification(){
		$this->db->where('tbl_document_verification.id',$this->uri->segment(2));
		$this->db->where('tbl_document_verification.is_deleted','0'); 
		$result = $this->db->get('tbl_document_verification');
		return $result->row();
	}
	...
	public function add_course_work_exam(){
		...
		$data=array(
			...
			'stream'           =>$this->input->post('stream') ?: null,
			'exam_name'        =>$exam_name,
	...
	public function get_common_course_work_exam_list_ajax($length,$start,$search){
		...
		$this->db->where('tbl_course_work_exam.is_deleted','0');
		// $this->db->where('tbl_course_work_exam.stream','1');
		$this->db->join("tbl_course","tbl_course.id = tbl_course_work_exam.course");
		$this->db->limit($length,$start);
		$this->db->order_by('tbl_course_work_exam.id','DESC');
		$result = $this->db->get('tbl_course_work_exam');
	...
	public function get_selected_stream_new($course){
		$this->db->select('tbl_course_stream_relation.*,tbl_stream.stream_name as stream');
		$this->db->where('tbl_course_stream_relation.is_deleted','0');
	...
	// } 
	public function get_single_course_data(){
	    $this->db->select('tbl_phd_course_work.*,tbl_student.enrollment_number,tbl_stream.stream_name,tbl_course_work_result.review_report,tbl_course_work_result.course_work_type,tbl_course_work_result.issue_date,tbl_course_work_result.issue_status,tbl_course_work_result.attending_status');
		$this->db->where('tbl_phd_course_work.is_deleted','0');
		$this->db->where('tbl_phd_course_work.status','1');
		$this->db->where('tbl_phd_course_work.id',$this->uri->segment(2));
		$this->db->join("tbl_student","tbl_student.id = tbl_phd_course_work.registration_id");
		$this->db->join("tbl_course_work_result","tbl_course_work_result.student_id = tbl_student.id","left");
		$this->db->join("tbl_stream","tbl_stream.id = tbl_phd_course_work.stream_id");
		$result = $this->db->get('tbl_phd_course_work');
		return $result->row();
	} 
	public function get_single_course_work(){ 
		$this->db->select('tbl_course_work_result.*,tbl_student.student_name,tbl_student.enrollment_number,tbl_signature.name as signature_name,tbl_signature.signature,tbl_signature.dispalay_signature,prepared_tbl_signature.name as prepared_signature_name,prepared_tbl_signature.signature as prepared_signature,prepared_tbl_signature.dispalay_signature as prepared_dispalay_signature,tbl_checked_signature.name as checked_signature_name,tbl_checked_signature.signature as checked_signature,tbl_checked_signature.dispalay_signature as checked_dispalay_signature');
	...
	public function update_course_work_result_issue_date(){
		$data = array(
			'issue_date' 			=> date("Y-m-d",strtotime($this->input->post('issue_date'))),
			'course_work_type' 		=> $this->input->post('course_work_type'),
			'prepared_signature_id' =>$this->input->post('prepared_signature'),
			'checked_signature_id'  =>$this->input->post('checked_signature'),
			'signature_id'  		=>$this->input->post('signature'),
		);
		$this->db->where('id',$this->input->post('id'));
		$this->db->update('tbl_course_work_result',$data);
	...
	public function update_blended_course_work_result_issue_date(){
		$data = array(
			'issue_date' => date("Y-m-d",strtotime($this->input->post('issue_date'))),
			'course_work_type' 		=> $this->input->post('course_work_type'),
			'prepared_signature_id' =>$this->input->post('prepared_signature'),
		...
		$this->db->where('id',$this->input->post('id'));
		$this->db->update('tbl_separate_student_course_work_result',$data);
		return true;
	}
	public function get_single_blended_course_work(){ 
		$this->db->select('tbl_separate_student_course_work_result.*,tbl_separate_student.student_name,tbl_separate_student.enrollment_number,tbl_signature.name as signature_name,tbl_signature.signature,tbl_signature.dispalay_signature,prepared_tbl_signature.name as prepared_signature_name,prepared_tbl_signature.signature as prepared_signature,prepared_tbl_signature.dispalay_signature as prepared_dispalay_signature,tbl_checked_signature.name as checked_signature_name,tbl_checked_signature.signature as checked_signature,tbl_checked_signature.dispalay_signature as checked_dispalay_signature');
		...
		$this->db->join("tbl_signature","tbl_signature.id = tbl_separate_student_course_work_result.signature_id",'left');
		$this->db->join("tbl_signature as prepared_tbl_signature","prepared_tbl_signature.id = tbl_separate_student_course_work_result.prepared_signature_id",'left');
		$this->db->join("tbl_signature as tbl_checked_signature","tbl_checked_signature.id = tbl_separate_student_course_work_result.checked_signature_id",'left');
	...
	public function get_course_work_result_list($length,$start,$search){ 
		$this->db->select('tbl_course_work_result.*,tbl_student.student_name,tbl_student.enrollment_number');
		if($search !=""){
			$this->db->group_start(); 
			$this->db->or_like('student_name',$search);
			$this->db->or_like('tbl_course_work_result.attending_status',$search);
			$this->db->or_like('tbl_course_work_result.issue_date',$search);
			$this->db->or_like('tbl_course_work_result.issue_status',$search);
			$this->db->or_like('tbl_course_work_result.review_report',$search);
			$this->db->or_like('tbl_student.enrollment_number',$search);
			$this->db->group_end(); 
		}
		$this->db->where('tbl_course_work_result.is_deleted','0');
		$this->db->where('tbl_course_work_result.status','1');
		$this->db->join("tbl_student","tbl_student.id = tbl_course_work_result.student_id");
		$this->db->order_by('tbl_course_work_result.id','DESC');
		$this->db->limit($length,$start);
		$result = $this->db->get('tbl_course_work_result');
		return $result->result();
	}
	...
	public function get_single_marksheet_result($id){
		$this->db->select('tbl_course_work_result.*,tbl_session.session_name,tbl_student.student_name,tbl_student.father_name,tbl_student.enrollment_number,tbl_stream.stream_name,tbl_signature.name as signature_name,tbl_signature.signature,tbl_signature.dispalay_signature,prepared_tbl_signature.name as prepared_signature_name,prepared_tbl_signature.signature as prepared_signature,prepared_tbl_signature.dispalay_signature as prepared_dispalay_signature,tbl_checked_signature.name as checked_signature_name,tbl_checked_signature.signature as checked_signature,tbl_checked_signature.dispalay_signature as checked_dispalay_signature');
        $this->db->where('tbl_course_work_result.is_deleted','0');
        $this->db->where('tbl_course_work_result.status','1');
	...
	public function get_bulk_hallticket_activate(){
		$selectedId = $this->input->post('slecter');
// 		echo '<pre>'; print_r($selectedId); exit;
	...
	public function bulk_marksheet_activation(){ 
		...
		if(is_array($resukt_id) && !empty($resukt_id)){
			...
			if(!empty($ids)){
				...
				if(!empty($result)){
					foreach($result as $single_result){
						...
						$student = $this->db->get('tbl_student');
						$student = $student->row();					
						if($student->year_sem == "1"){
							$exist_rr = array(
								'name' => 'name'
							);
						}else{
							$exist_rr = $this->get_re_registered_student_year_sem($student->enrollment_number,$student->year_sem); 
						}
						...
						if(!empty($exist_rr)){  
							...
							$this->db->where('student_id',$student->id);
							$this->db->where('year_sem',$single_result->year_sem);
							$this->db->where('is_deleted','0');
							$marksheet_already = $this->db->get('tbl_marksheet');	
							$marksheet_already = $marksheet_already->row(); 
							if(empty($marksheet_already)){
								$this->db->insert('tbl_marksheet',$data);
								$last_id = $this->db->insert_id();
								$update = array(
									'marksheet_number' => $marksheet_number.$last_id,
								...
								$this->db->where('id',$marksheet_already->id); 
								$this->db->update('tbl_marksheet',$data);
							}
						}	
					} 
	...
	public function delete_selected_result(){ 
		$ids = array();
		$resukt_id = $this->input->post('resukt_id'); 
	...
	public function get_single_course_data_Separate_student(){
	    $this->db->select('tbl_separate_student_phd_course_work.*,tbl_separate_student.enrollment_number,tbl_stream.stream_name,tbl_separate_student_course_work_result.review_report,tbl_separate_student_course_work_result.course_work_type,tbl_separate_student_course_work_result.issue_date,tbl_separate_student_course_work_result.issue_status,tbl_separate_student_course_work_result.attending_status');
		...
		$this->db->join("tbl_separate_student","tbl_separate_student.id = tbl_separate_student_phd_course_work.registration_id");
		$this->db->join("tbl_stream","tbl_stream.id = tbl_separate_student_phd_course_work.stream_id");
		$this->db->join("tbl_separate_student_course_work_result","tbl_separate_student_course_work_result.student_id = tbl_separate_student.id","left");
		$result = $this->db->get('tbl_separate_student_phd_course_work');
		return $result->row();
	} 
	public function set_course_work_result_seperate_student($file1){ 
		if($file1 == ""){
			$file1 = $this->input->post("review_report");
		}
		...
		$this->db->insert('tbl_separate_student_course_work_result',$data);
		$id  = $this->db->insert_id();
		$marksheet_number_ref = '';
		if(strlen($id) == 1){
			$marksheet_number_ref = "0000000".$id;
		...
		// echo "<pre>";print_r($marksheet_number);exit;
		$this->db->where('id',$id);
		$this->db->update('tbl_separate_student_course_work_result',$marksheet_number);
		...
		for($i=1;$i<=3;$i++){ 
 		...
 		if(!empty($details_result)){
			$this->db->insert_batch('tbl_separate_course_work_result_details',$details_result);
		}
		return true;
	}
	public function get_course_work_result_list_separate_student($length,$start,$search){
		$this->db->select('tbl_separate_student_course_work_result.*,tbl_separate_student.student_name');
		$this->db->where('tbl_separate_student_course_work_result.is_deleted','0');
		if($search !=""){
			$this->db->group_start(); 
			$this->db->or_like('tbl_separate_student.student_name',$search); 
			$this->db->or_like('tbl_separate_student_course_work_result.attending_status',$search);			
			$this->db->or_like('tbl_separate_student_course_work_result.issue_date',$search); 			
			$this->db->or_like('tbl_separate_student_course_work_result.issue_status',$search); 
			$this->db->group_end();
		}
		$this->db->where('tbl_separate_student_course_work_result.is_deleted','0');
		$this->db->where('tbl_separate_student_course_work_result.status','1');
		$this->db->join("tbl_separate_student","tbl_separate_student.id = tbl_separate_student_course_work_result.student_id");
		$this->db->order_by('tbl_separate_student_course_work_result.id','DESC');
		$this->db->limit($length,$start);
		$result = $this->db->get('tbl_separate_student_course_work_result');
		return $result->result();
	}
	public function get_course_work_result_list_separate_student_count($search){
		$this->db->select('tbl_separate_student_course_work_result.*,tbl_separate_student.student_name,tbl_separate_student.enrollment_number');
		$this->db->where('tbl_separate_student_course_work_result.is_deleted','0'); 
		if($search !=""){
			$this->db->group_start();
			$this->db->or_like('tbl_separate_student.student_name',$search); 
	...
	public function get_single_marksheet_result_seprate_student($id){
		$this->db->select('tbl_separate_student_course_work_result.* ,tbl_separate_student.student_name,tbl_separate_student.father_name,tbl_separate_student.enrollment_number,tbl_stream.stream_name,tbl_signature.name as signature_name,tbl_signature.signature,tbl_signature.dispalay_signature,prepared_tbl_signature.name as prepared_signature_name,prepared_tbl_signature.signature as prepared_signature,prepared_tbl_signature.dispalay_signature as prepared_dispalay_signature,tbl_checked_signature.name as checked_signature_name,tbl_checked_signature.signature as checked_signature,tbl_checked_signature.dispalay_signature as checked_dispalay_signature');
        $this->db->where('tbl_separate_student_course_work_result.is_deleted','0');
        $this->db->where('tbl_separate_student_course_work_result.status','1');
        $this->db->where('tbl_separate_student_course_work_result.id',$id);
		$this->db->join('tbl_separate_student','tbl_separate_student.id = tbl_separate_student_course_work_result.student_id');
	...
	public function set_online_verification(){ 
		$document = "";
		if($_FILES['document']['name'] !=""){
			$document = $this->Digitalocean_model->upload_photo($filename="document",$path="uploads/document_verification/");
		}
		if($document == ""){
			$document = $this->input->post('old_document');
		}
		$data = array(
			'name_of_department' 	=> $this->input->post('name_of_department'),
		...
		$this->db->update('tbl_document_verification',$data);
		$fileName = ""; 
		if($_FILES['other_document_file']['name'][0] !=""){
			$fileName = $this->Digitalocean_model->upload_photo_multiple($filename="other_document_file",$path="uploads/document_verification/");
		}
		$exp = explode(",",$fileName);
		$other_document_name = $this->input->post('other_document_name');
		if($fileName !=""){
			for($i=0;$i<count($other_document_name);$i++){
				if($other_document_name[$i] !=""){
					$data2 = array(
						'verification_id' 	=> $this->input->post('hidden_id'),
						'document_name' 	=> $other_document_name[$i],
						'document_file' 	=> $exp[$i],
						'created_on' 		=> date("Y-m-d H:i:s"),
					);
					$this->db->insert('tbl_verification_documents_details',$data2);
					$this->session->set_flashdata('success','Record Inserted Successfully...');   //--
				}
			}
		}
		if($this->input->post('sendmail') == "sendmail"){
			$message = $this->input->post('mail_content');  
			$to = array(
        		"email" => $this->input->post('mail_ids'),
        		"name" 	=> $this->security->xss_clean($this->input->post('name_of_department')),
        	);
        	$subject = 'Verification Details |THE GLOBAL UNIVERSITY';    
            $this->Admin_model->send_send_blue($to,$subject,$message);
			$this->session->set_flashdata('success','Mail Send Successfully...');   //--
		}
		return true;
	}
	public function get_verified_documents(){
		$this->db->where('verification_id',$this->uri->segment(2));
		$this->db->where('is_deleted','0');
		$result = $this->db->get('tbl_verification_documents_details');
		return $result->result();
	}
	public function set_offline_document_verification($documents){
		//echo "<pre>";print_r($_POST);exit;
		$data = array(
		...
		if($this->input->post('id') == ""){
			...
			$new_arr = array_merge($data,$date);
			$this->db->insert("tbl_document_verification",$new_arr);
			$id = $this->db->insert_id();
		}else{
			$this->db->where('id',$this->input->post('id'));
			$this->db->update("tbl_document_verification",$data);
		...
		redirect("offline_document_verification_list");
	}
	public function get_verified_student(){
		...
		// 	$result = $result->row();
		// 	if(!empty($result)){
		// 		$this->session->set_flashdata('message','You have already filled examination form');
		// 		redirect('admin_fill_exam_form');
		// 	}else{
		// 		redirect('admin_fill_exam_form/'.$result->student_id);
		// 	}
		// }else{
		// 	$this->session->set_flashdata('message','Please enter valid enrollment number');
		// 	redirect('admin_fill_exam_form');
		// }
		// if(!empty($result->admission_status) && $result->admission_status == "4"){
		// 	$this->session->set_flashdata('message','This student is already passout');
		// 	redirect('admin_fill_exam_form');
		// }else 
		if(!empty($result)){
			redirect('admin_fill_exam_form/'.$result->id);
		}else{
			$this->session->set_flashdata('message','Please enter valid enrollment number');
			redirect('admin_fill_exam_form');
		}
	...
	public function get_this_year_assignement($student,$year_sem){
		$this->db->where('is_deleted','0');
		$this->db->where('status','1'); 
	...
	public function get_all_answer_book_ajax($length,$start,$search){
		$this->db->select('tbl_center_answer_book.*,tbl_course.course_name,tbl_stream.stream_name,tbl_center.center_name,tbl_subject.subject_name,tbl_student.student_name,tbl_student.enrollment_number');
		...
		$this->db->join('tbl_center','tbl_center.id = tbl_center_answer_book.center_id');
	...
	public function get_all_answer_book_ajax_count($search){
		$this->db->select('tbl_center_answer_book.*,tbl_course.course_name,tbl_stream.stream_name,tbl_center.center_name');
		$this->db->where('tbl_center_answer_book.is_deleted', '0'); 
		if($search !=""){
			$this->db->group_start(); 
            $this->db->or_like('tbl_center_answer_book.title',$search); 
	...
	public function set_for_print_course_work_result(){ 
		$ids = $this->input->post('ids'); 
		if(!empty($ids)){
			for($i=0;$i<count($ids);$i++){
				$data = array(
	...
	public function get_single_marksheet_for_duplicate(){
		$this->db->select('tbl_marksheet.*,tbl_student.student_name');
		$this->db->where('tbl_marksheet.id',$this->uri->segment(2));
		$this->db->join('tbl_student','tbl_student.id = tbl_marksheet.student_id');
		$result = $this->db->get('tbl_marksheet');
		return $result->row();
	}
	public function set_duplicate_marksheet($document){
		if($document == ""){
			$document = $this->input->post('old_document');
		}
		$data = array(
			'marksheet_id' 		=> $this->input->post('id'), 
			'student_id' 		=> $this->input->post('student_id'),
			'year_sem' 			=> $this->input->post('year_sem'), 
			'reference_number' 	=> '', 
			'remark' 			=> $this->input->post('remark'), 
		...
		$this->db->insert('tbl_duplicate_marksheet',$data);
	}
	public function get_all_duplicate_marksheet(){
		$this->db->select('tbl_marksheet.*,tbl_duplicate_marksheet.id as duplicate_id,tbl_duplicate_marksheet.sent_for_print as sent_for_print_new,tbl_duplicate_marksheet.sent_for_print,tbl_student.student_name,tbl_student.enrollment_number,tbl_course.course_name,tbl_stream.stream_name,tbl_exam_results.result,tbl_exam_results.examination_month,tbl_exam_results.examination_year');
		if($this->input->post('month') != ""){
			$this->db->where('tbl_exam_results.examination_month',$this->input->post('month'));
		}
		if($this->input->post('year') != ""){
		...
		$this->db->where('tbl_exam_results.is_deleted','0');
		$this->db->where('tbl_exam_results.status','1');
		$this->db->join('tbl_marksheet','tbl_marksheet.id = tbl_duplicate_marksheet.marksheet_id');
		$this->db->join('tbl_exam_results','tbl_exam_results.id = tbl_marksheet.result_id');
		$this->db->join('tbl_student','tbl_student.id = tbl_marksheet.student_id');
		$this->db->join('tbl_course','tbl_course.id = tbl_student.course_id');
		$this->db->join('tbl_stream','tbl_stream.id = tbl_student.stream_id');
		$result = $this->db->get('tbl_duplicate_marksheet');
		return $result->result();
	}
	
	public function set_time_table(){ 
		$str_exp = explode("@@@",$this->input->post('stream'));
		$data = array(
			'course' 		=> $this->input->post('course'),
			'stream' 		=> $str_exp[1],
			'year_sem' 		=> $this->input->post('year_sem'),
		...
		$this->db->insert('tbl_time_table',$data);
		$last_id = $this->db->insert_id();
		$subject = $this->input->post('subject');
		...
		if(!empty($details)){
			$this->db->insert_batch('tbl_timesheet_details',$details);
		}
		return true;
	}
	public function get_searched_timesheet(){
		$this->db->select('tbl_time_table.*,tbl_course.print_name,tbl_stream.stream_name');
		if(isset($_GET['course']) && $_GET['course'] !=""){
			$this->db->where('tbl_time_table.course',$_GET['course']);
		}
		if(isset($_GET['stream']) && $_GET['stream'] !=""){
			$exp = explode("@@@",$_GET['stream']);
			$this->db->where('tbl_time_table.stream',$exp[1]);
		}
		if(isset($_GET['year_sem']) && $_GET['year_sem'] !=""){
			$this->db->where('tbl_time_table.year_sem',$_GET['year_sem']);
		}
		if(isset($_GET['exam_month']) && $_GET['exam_month'] !=""){
			$this->db->where('tbl_time_table.exam_month',$_GET['exam_month']);
		}
		if(isset($_GET['exam_year']) && $_GET['exam_year'] !=""){
			$this->db->where('tbl_time_table.exam_year',$_GET['exam_year']);
		}
		$this->db->where('tbl_time_table.is_deleted','0');
		$this->db->join('tbl_course','tbl_course.id = tbl_time_table.course');
		$this->db->join('tbl_stream','tbl_stream.id = tbl_time_table.stream','left');
		$this->db->order_by('tbl_time_table.id','DESC');
		...
		if(!empty($result)){
			foreach($result as $result_arr){
				$this->db->where('is_deleted','0');
				$this->db->where('sheet_id',$result_arr->id);
				$details = $this->db->get('tbl_timesheet_details');
				$details = $details->result();
				$result_arr->data = $details;
				$new_arr[] =  $result_arr;
			}
		}
		return $new_arr;
	}
	public function get_last_pass_year($enrollment){
		$this->db->where('enrollment_number',$enrollment);
		$this->db->where('is_deleted','0');
	...
	public function get_signature(){
		$this->db->where('tbl_signature.is_deleted','0');
		$this->db->where('tbl_signature.status','1');
		$this->db->where('tbl_signature.id',$this->input->post('signature'));
		$result = $this->db->get('tbl_signature');
		$result = $result->row();
		$signature_img = '';
		if(!empty($result) && $result->signature != ""){
			$signature_img = $this->Digitalocean_model->get_photo('certificate_signature/'.$result->signature);
		}
		echo $signature_img;
	}
	
	public function get_exam_setup($month,$year){
		$this->db->where('month',$month);
		$this->db->where('year',$year);
		$this->db->where('is_deleted','0');
		$this->db->where('student_type','0');
		$result = $this->db->get('tbl_exam_setup')->row();
		return $result;
	}
	public function check_student_examination_history(){
		$this->db->where('is_deleted','0');
		$this->db->where('enrollment_number',$this->input->post('enrollment_number'));
		$result = $this->db->get('tbl_student');
		$result = $result->row();
		if(!empty($result)){
			redirect('student_examination_history_activity/'.$result->id);
		}else{
			return false;
		}
	}
	public function get_student_all_examination_activities(){
		$enrollment = 0;
		$activity = array();
    ...
    }		public function get_student_total_cgpa($student_id){		echo "<pre>";		print_r($student_id);exit;		$this->db->select('tbl_marksheet.*,tbl_signature.name as signature_name,tbl_signature.signature,tbl_signature.dispalay_signature,tbl_exam_results.result,tbl_exam_results.examination_month,tbl_exam_results.examination_year,tbl_student.mother_name,tbl_student.student_name,tbl_student.father_name,tbl_student.center_id,tbl_student.date_of_birth,tbl_student.year_sem,tbl_student.course_mode,tbl_course.course_name,tbl_stream.stream_name,tbl_session.session_name,tbl_session.pre_session,tbl_course.course_name,tbl_course.id as course_id,prepared_tbl_signature.name as prepared_signature_name,prepared_tbl_signature.signature as prepared_signature,prepared_tbl_signature.dispalay_signature as prepared_dispalay_signature,tbl_checked_signature.name as checked_signature_name,tbl_checked_signature.signature as checked_signature,tbl_checked_signature.dispalay_signature as checked_dispalay_signature');		$this->db->where('tbl_marksheet.is_deleted','0');		$this->db->where('tbl_marksheet.student_id',$student_id);		$this->db->join('tbl_exam_results','tbl_exam_results.id = tbl_marksheet.result_id');		$this->db->join('tbl_student','tbl_student.id = tbl_marksheet.student_id');		$this->db->join('tbl_course','tbl_course.id = tbl_student.course_id');		$this->db->join('tbl_session','tbl_session.id = tbl_student.session_id');		$this->db->join('tbl_stream','tbl_stream.id = tbl_student.stream_id');		$this->db->join('tbl_signature','tbl_signature.id = tbl_marksheet.signature_id','left');		$this->db->join("tbl_signature as prepared_tbl_signature","prepared_tbl_signature.id = tbl_marksheet.prepared_signature_id",'left');		$this->db->join("tbl_signature as tbl_checked_signature","tbl_checked_signature.id = tbl_marksheet.checked_signature_id",'left');		$result = $this->db->get('tbl_marksheet');		$result = $result->result();			}
} 